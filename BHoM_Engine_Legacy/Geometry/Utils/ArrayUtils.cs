...
namespace BH.oM.Geometry
{
    public static class ArrayUtils
    {
        ...
        internal static double[] Intersect(double[] s1, double[] v1, double[] s2, double[] v2, bool bound)
        {
            ...
            double[] rhs = ArrayUtils.Sub(s2, s1);
            int tries = 0;
            while (tries++ < 2)
            {
                ...
                if (!double.IsInfinity(multiplier))
                {
                    ...
                    return double.IsNaN(t) || bound && (t > 1 || t < 0) ? null : ArrayUtils.Add(s2, ArrayUtils.Multiply(v2, t));
                }
                else
                {
                    int temp = a2;
        ...
        internal static double[] Intersect(double[] s1, double[] v1, double[] s2, double[] v2)
        {
            ...
            while (tries++ < 2)
            {
                ...
                {
                    double t = (rhs[a2] * multiplier - rhs[a1]) / (v2[a1] - v2[a2] * multiplier);
                    return ArrayUtils.Add(s2, ArrayUtils.Multiply(v2, t));
                }
                else
                {
                    int temp = a2;
        ...
        internal static double PointLineDistance(double[] pl1, double[] pl2, double[] p)
        {
            ...
            double[] pointLineNormal = ArrayUtils.CrossProduct(vector, Sub(p, pl1));
            double[] p3 = Add(pl1, pointLineNormal);
            double[] normal = ArrayUtils.CrossProduct(vector, Sub(p3, pl1));
            double D = -ArrayUtils.DotProduct(normal, pl1);
            return (ArrayUtils.DotProduct(normal, p) + D) / ArrayUtils.Length(normal);
        }
        ...
        {
            double length = ArrayUtils.Length(m_Coordinates);
        ...
        {
            double angle = ArrayUtils.Angle(v1, v2);
