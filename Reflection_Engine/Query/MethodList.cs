...
namespace BH.Engine.Reflection
{
    public static partial class Query
    {
            ...
            foreach (Assembly asm in AppDomain.CurrentDomain.GetAssemblies())
            {
                try
                {
                    ...
                    else if (name.EndsWith("oM") || name.EndsWith("_Adapter") || name.EndsWith("_UI") || name.EndsWith("_Test"))
                    {
                        foreach (Type type in asm.GetTypes())
                        {
                            bool isAdapterMethod = false;
                            if (name.EndsWith("_Adapter"))
                                isAdapterMethod = true;

                            StoreAllMethods(type, isAdapterMethod);
                        }
                    }
                }
                catch (Exception)
        ...
        private static void StoreAllMethods(Type type, bool isAdapterMethod = false)
        {
            ...
            if (type.Namespace != null && type.Namespace.StartsWith("BH.") && !type.IsAutoGenerated())
            {
                ...
                if (isAdapterMethod)
                    m_AdapterMethodsList.AddRange(type.GetMethods(bindingAll).Where(x => x.GetMethodBody() != null && !x.IsAutoGenerated()));
            }
        }
