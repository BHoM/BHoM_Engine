...
namespace BHoM.Geometry
{
    public static partial class Create
    {
        ...
        private static Arc Initialise(this Arc arc, double startAngle, double endAngle, double radius, Plane plane)
        {
            ...
            double rotationAngle = ArrayUtils.Angle(plane.Normal, BHoM.Geometry.Vector.ZAxis());
            if (rotationAngle > 0)
            {
                Vector rotationAxis = BHoM.Geometry.Vector.CrossProduct(plane.Normal, BHoM.Geometry.Vector.ZAxis());
                Transform t = Transform.Rotation(BHoM.Geometry.Point.Origin, rotationAxis, rotationAngle);
                controlPoints = ArrayUtils.MultiplyMany(t, controlPoints);
            }
            controlPoints = ArrayUtils.MultiplyMany(Transform.Translation(plane.Origin - BHoM.Geometry.Point.Origin), controlPoints);
