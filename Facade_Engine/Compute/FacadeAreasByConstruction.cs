...
namespace BH.Engine.Facade
{
    public static partial class Compute
    {
        ...
        [Description("Returns total areas of each unique Construction contained in the provided elements, as well as the toal Frame area (Used for a panel containing openings with FameEdgeProperties).")]
        ...
        public static Output<List<double>, List <IConstruction>, double> FacadeAreasByConstruction(this List<IFacadeObject> elems)
        {
            ...
            Dictionary<IConstruction, double> areasDict = new Dictionary<IConstruction, double>();
            double frameArea = 0;
            ...
            foreach (IFacadeObject elem in uniqueElems)
            {
                if (elem is Panel)
                {
                    ...
                    if (areasDict.Keys.Contains(panel.Construction))
                    {
                        double prevArea = areasDict[panel.Construction];
                        areasDict[panel.Construction] = prevArea + area;
                    }
                    else
                    {
                        areasDict.Add(panel.Construction, area);
                    }
                }
                else if (elem is Opening)
                {
                    ...
                    if (areasDict.Keys.Contains(opening.OpeningConstruction))
                    {
                        double prevArea = areasDict[opening.OpeningConstruction];
                        areasDict[opening.OpeningConstruction] = prevArea + openArea;
                    }
                    else
                    {
                        areasDict.Add(opening.OpeningConstruction, openArea);
                    }
                }
            }
            
            List<IConstruction> constructions = areasDict.Keys.ToList();
            List<double> areas = areasDict.Values.ToList();

            return new Output<List<double>, List<IConstruction>, double>
            {
                Item1 = areas,
                Item2 = constructions,
                Item3 = frameArea
