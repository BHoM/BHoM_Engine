...
namespace BH.Engine.Environment
{
    public static partial class Query
    {
        ...
        public static Point BottomLeft(this Panel panel, List<Panel> panelsAsSpace)
        {
            if(panel == null)
            {
                BH.Engine.Base.Compute.RecordError("Cannot query the bottom left corner of a null panel.");
                return null;
            }

            if(panelsAsSpace == null)
            {
                BH.Engine.Base.Compute.RecordError("Cannot query the bottom left corner of a panel without knowing the space it belongs to.");
                return null;
            }

            return panel.Polyline().BottomLeft(panelsAsSpace);
        }

        [Description("Gets the bottom left most point of a polyline when looking from within the space to the outside")]
        [Input("polyline", "The BHoM Geometry Polyline to get the bottom left most point of")]
        [Input("panelsAsSpace", "A collection of Environment Panels representing a single space")]
        [Output("bottomRightPoint", "The bottom left most point of the panel")]
        public static Point BottomLeft(this Polyline polyline, List<Panel> panelsAsSpace)
        {
            if (polyline == null)
            {
                BH.Engine.Base.Compute.RecordError("Cannot query the bottom left corner of a null polyline.");
                return null;
            }

            if (panelsAsSpace == null)
            {
                BH.Engine.Base.Compute.RecordError("Cannot query the bottom left corner of a polyline without knowing the space it belongs to.");
                return null;
            }
