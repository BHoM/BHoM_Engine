...
namespace BH.Engine.Structure
{
    public static partial class Create
    {
        ...
        [Input("perimeter", "The outer edge of the pad. All section constants are derived based on the dimensions of this.")]
        ...
        public static PadFoundation PadFoundation(PolyCurve perimeter, ISurfaceProperty thickness = null, double orientationAngle = 0)
        {
            return perimeter.IsNull() ? null : new PadFoundation() { Perimeter = perimeter, Property = thickness, OrientationAngle = orientationAngle };
        }
        ...
        {
            PolyCurve perimeter = Spatial.Create.RectangleProfile(length, width).Edges.ToList().IJoin()[0];

            if (coordinateSystem == null)
                coordinateSystem = new Cartesian();
            else
                perimeter.Orient(new Cartesian(), coordinateSystem);

            return PadFoundation(perimeter, thickness, orientationAngle);
        }
    }
}
