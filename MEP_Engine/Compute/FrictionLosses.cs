...
namespace BH.Engine.MEP
{
    public static partial class Compute
    {
        ...
        public static double FrictionLosses(double frictionFactor, double ductLength, double circularDiameter, double fluidDensity, double fluidVelocity)
        {
            ...
            {
                BH.Engine.Base.Compute.RecordError("Cannot compute the friction factor from a null friction factor.");
                return -1;
            }

            if(ductLength == double.NaN)
            {
                BH.Engine.Base.Compute.RecordError("Cannot compute the friction factor from a null duct length.");
                return -1;
            }
            ...
            {
                BH.Engine.Base.Compute.RecordError("Cannot compute the friction factor from a null fluid density.");
                return -1;
            }
            ...
            double frictionLosses = (1000 * frictionFactor*ductLength/circularDiameter) * (fluidDensity/2) * Math.Pow(fluidVelocity,2);
